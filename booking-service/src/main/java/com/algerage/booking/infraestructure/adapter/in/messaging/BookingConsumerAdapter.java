...
public class BookingConsumerAdapter implements BookingConsumerPort {
    ...
    public void consumeBooking(final BookingMessage message) {
        ...
        String normalizedTime = message.timeSlotId().replace(":", "-");
    ...
    @Override
    public void manageIncomingBooking(BookingMessage message) {
        log.info("Message consumed: {}", message);
        //System.out.println("Message consumed: " + message);
    ...
    private void lockBookingRequest(final BookingMessage bookingMessage) {
        log.info("Trying to lock booking request: {}", bookingMessage);

        String normalizedDate = bookingMessage.date().replace("/", "-");
        String normalizedTime = bookingMessage.timeSlotId().replace(":", "-");

        // Create the unique lock key
        String lockKey = String.format("lock:booking:%s:%s:%s",
                bookingMessage.clientId(), normalizedDate, normalizedTime);
        ...
        boolean paymentsuccess = false;
        try {
            // Try to acquire the lock (wait up to 600 seconds, auto-release after 60 seconds)
            isLocked = lock.tryLock(0, 600, TimeUnit.SECONDS);
            if (isLocked) {
                log.info("Lock acquired for: {}", lockKey);
                log.info("Generating payment parameters for: {}", bookingMessage);
                bookingAckProducer.sendBookingAck(new BookingMessageAck(bookingMessage, true, "Slot reserved but pending for payment"));
                ...
                log.warn("Lock not acquired for: {}", lockKey);
                bookingAckProducer.sendBookingAck(new BookingMessageAck(bookingMessage, false, "Slot already locked"));
            }
